DROP TRIGGER IF EXISTS `trbodyfeeling_delete`;
DELIMITER //
CREATE TRIGGER `trbodyfeeling_delete` BEFORE DELETE ON `tblbodyfeeling`
 FOR EACH ROW BEGIN
    UPDATE tbljournaloperation Set id = OLD.id, userid = OLD.userid, operationtypeid = 3, changeddate = CURRENT_TIMESTAMP where rowid = OLD.rowid;
END
//
DELIMITER ;
DROP TRIGGER IF EXISTS `trbodyfeeling_insert`;
DELIMITER //
CREATE TRIGGER `trbodyfeeling_insert` AFTER INSERT ON `tblbodyfeeling`
 FOR EACH ROW BEGIN
    INSERT INTO tbljournaloperation Set rowid = NEW.rowid, id = NEW.id, userid = NEW.userid, operationtypeid = 1; 
END
//
DELIMITER ;
DROP TRIGGER IF EXISTS `trbodyfeeling_update`;
DELIMITER //
CREATE TRIGGER `trbodyfeeling_update` AFTER UPDATE ON `tblbodyfeeling`
 FOR EACH ROW BEGIN
    UPDATE tbljournaloperation Set id = NEW.id, userid = NEW.userid, operationtypeid = 2, changeddate = CURRENT_TIMESTAMP where rowid = NEW.rowid;
END
//
DELIMITER ;