...
// Constants
//
const planet_o   = 16;         // Index offset
const planet_dim = 33;         // Work array dimension

//
// Types
//
class Enum_PlanetType extends Enum { 
	const SUN = 0;
	const MERCURY = 1; 
	const VENUS = 2;
	const EARTH = 3;
...
class PlanetPert {
	...
	public function init($T, $M_, $I_min_, $I_max_, $m, $i_min, $i_max, $phi = null){
		if($phi == null) $phi = 0.0;
		...
		$this->m_C_[planet_o] = cos($phi);
		$this->m_S_[planet_o] = sin($phi);
		
		for ($i = 0; $i < $I_max_; $i++) {
			AddThe ( $this->m_C_[planet_o + $i], $this->m_S_[planet_o + $i], +$this->m_cosM, +$this->m_sinM, $this->m_C_[planet_o + $i + 1], $this->m_S_[planet_o + $i + 1] ); 
		}
		for ($i = 0; $i > $I_min_; $i--) {
			AddThe ( $this->m_C_[planet_o + $i], $this->m_S_[planet_o + $i], +$this->m_cosM, -$this->m_sinM, $this->m_C_[planet_o + $i - 1], $this->m_S_[planet_o + $i - 1] ); 
		}
		...
		// cosine and sine of multiples of m
		$this->m_c[planet_o]=1.0; $this->m_c[planet_o + 1] = cos($m); $this->m_c[planet_o - 1] = +$this->m_c[planet_o + 1];
		$this->m_s[planet_o]=0.0; $this->m_s[planet_o + 1] = sin($m); $this->m_s[planet_o - 1] = -$this->m_s[planet_o + 1];
		
		for ($i = 1; $i < $i_max; $i++) {
			AddThe ( $this->m_c[planet_o + $i], $this->m_s[planet_o + $i], $this->m_c[planet_o + 1], $this->m_s[planet_o + 1], $this->m_c[planet_o + $i + 1], $this->m_s[planet_o + $i + 1] );
		}
		for ($i = -1; $i > $i_min; $i--) {
			AddThe ( $this->m_c[planet_o + $i], $this->m_s[planet_o + $i], $this->m_c[planet_o - 1], $this->m_s[planet_o - 1], $this->m_c[planet_o + $i - 1], $this->m_s[planet_o + $i - 1] );
		}
	...
	public function Term ( $I_, $i, $iT, $dlc, $dls, $drc, $drs, $dbc, $dbs ) {
		if ($iT == 0) {
			AddThe ( $this->m_C_[planet_o + $I_], $this->m_S_[planet_o + $I_], $this->m_c[planet_o + $i], $this->m_s[planet_o + $i], $this->m_u, $this->m_v );
		} else {
			$this->m_u *= $this->m_T;
			$this->m_v *= $this->m_T; 
		}
...
//
//------------------------------------------------------------------------------
function PertPosition (Enum_PlanetType $Planet, $T)
{
	$r = Vec3D::GetNewInstance(); // Null vector;

	switch ( $Planet->toInt() ){
	case Enum_PlanetType::SUN:      break;
	case Enum_PlanetType::MERCURY:  $r = MercuryPos($T);  break;
	case Enum_PlanetType::VENUS:    $r = VenusPos($T);    break;
	case Enum_PlanetType::EARTH:    $r = Vec3D::UnaryMinusOfVector(SunPos($T));  break;
	case Enum_PlanetType::MARS:     $r = MarsPos($T);     break;
	case Enum_PlanetType::JUPITER:  $r = JupiterPos($T);  break;
	case Enum_PlanetType::SATURN:   $r = SaturnPos($T);   break;
	case Enum_PlanetType::URANUS:   $r = UranusPos($T);   break;
	case Enum_PlanetType::NEPTUNE:  $r = NeptunePos($T);  break;
	case Enum_PlanetType::PLUTO:    $r = PlutoPos($T);
	}
...
function MercuryPos ($T)
{
	...
	// Variables
	//
	$M1 = null;          // Mean anomalies
	$M2 = null;
	$M3 = null;
	$M5 = null;
	$M6 = null;
	$Ven = new PlanetPert();      // Perturbations
	$Ear = new PlanetPert();
	$Jup = new PlanetPert();
	$Sat = new PlanetPert();
	$dl = null;              // Corrections in longitude ["],
	$dr = null;			     // radius [AU] and latitude ["]
	$db = null;

	$l = null;               // Ecliptic coordinates
	$b = null;
	$r = null;
...
function VenusPos ($T)
{
	...
	$M5 = null;
	$M6 = null;
	$Mer = new PlanetPert();     // Perturbations
	$Ear = new PlanetPert();
	$Mar = new PlanetPert();
	$Jup = new PlanetPert();
	$Sat = new PlanetPert();
...
function JupiterPos ($T)
{
	...
	// Variables
	//
	$M5 = null;                // Mean anomalies
	$M6 = null;
	$M7 = null;
	$Sat = new PlanetPert();                 // Perturbations
	$Ura = new PlanetPert();
	$phi = null;
	$c = null;
	$s = null;
...
function SaturnPos ($T)
{
	...
	$M7 = null;
	$M8 = null;
	$Jup = new PlanetPert();             // Perturbations
	$Ura = new PlanetPert();
	$Nep = new PlanetPert();
	$phi = null;
	$c = null;
...
function NeptunePos ($T)
{
	...
	$M6 = null;
	$M7 = null;
	$M8 = null;             // Mean anomalies
	$Jup = new PlanetPert();
	$Sat = new PlanetPert();
	$Ura = new PlanetPert();             // Perturbations
	$dl = null; 
	$dr = null; 
	$db = null;              // Corrections in longitude ["],
								   // radius [AU] and latitude ["]
	$l = null;
	$b = null;
	$r = null;                   // Ecliptic coordinates
...
function PlutoPos ($T)
{
	...
	$b = null;
	$r = null;
	$R = Vec3D::getNewInstance();                       // Position vector
	...
	$R = Vec3D::getNewInstance2 (new Polar($l,$b,$r) );

	// Position vector; ecliptic and equinox of date
	return Vec3D::MatrixVectorProduct(PrecMatrix_Ecl(T_B1950,$T), $R);
}
...
//
//------------------------------------------------------------------------------
function KepVelocity (Enum_PlanetType $Planet, $T)
{
  //
  // Variables
  //
...
//
//------------------------------------------------------------------------------
function State (Enum_PlanetType $Planet, $T, &$r, &$v)
{
	//
	// Constants
	//
	$p = 1.3970;        // Centential precession [deg]
	...
	$r = Vec3D::MatrixVectorProduct($PQR,$r);                                        // Ecliptic
	$v = Vec3D::MatrixVectorProduct($PQR,$v);                                        // coordinates
