		...
		for($i = 0; $i < count($phases); $i++) {
			if(substr($phases[$i][2],0,1) == "s"){
				...
				if($LC->Phase->toInt() > (new Enum_enPhase("NOECLIPSE"))->toInt()){
					...
					$eclipse = array("p" => $LC->Phase->toInt(),
									 "m" => $LC->Mag,
									 "max" => date("Y-m-d H:i:s",$maximumTime),
									 "c1" => date("Y-m-d H:i:s",$contact1),
									 "c2" => $contact2 == null ? null : date("Y-m-d H:i:s",$contact2),
									 "c3" => $contact3 == null ? null : date("Y-m-d H:i:s",$contact3),
									 "c4" => date("Y-m-d H:i:s",$contact4),
									 "cid" => $cityId);
					$eclipses[] = $eclipse;
				}
			}
